...
int insertMode(char text[MAXBARIS][MAXKOLOMARR], int jmlBaris)
{
	...
	do
	{
		...
		case CTRL_L: /*Pindah cursor ke kanan*/
			...
			if (x == curEOL && y + 1 < amountOfLine)
			{ /*Jika curosr pada ujung kanan*/
				x = 0;
				y++;
			}
			else if (x < curEOL)
		...
		case CTRL_I: /*Pindah cursor ke atas*/
			...
			if (y > 0)
			{
				...
				{ /*Jika panjang baris atas lebih pendek*/
					y--;
					x = prevEOL;
				}
			}
		...
		case CTRL_K: /*Pindah cursor ke bawah*/
			...
			if (y + 1 < amountOfLine)
			{ // y+1 karena amountOfLine mulainya dari 1 & y mulai dari 0
				if (x <= nextEOL)
				{
					y++;
				}
				else
				{ /*Jika garis bawah lebih panjang*/
					y++;
					x = nextEOL;
				}
			}
		...
		case ENTER: /*Tambah baris baru*/
			...
			{
				if (x == curEOL)
				{ /*Jika cursor berada di akhir baris*/
					x = 0;
					if (y + 1 < amountOfLine)
					{											/*Jika dibawah cursor terdapat baris lain*/
					...
					{ /*Jika dibawah cursor tidak terdapat baris lain*/
						amountOfLine++;
						text[y + 1][0] = NULL;
					}
					y++;
				...
				{ /*Jika cursor berada di awal baris*/
				...
				{ /*Jika cursor berada di tengah baris*/
					curEOL = findEndOfLine(text, y);
					...
					for (int i = x, j = 0; i < curEOL; i++, j++)
					{ /*Geser char di kanan cursor ke garis baru */
						text[y + 1][j] = text[y][i];
					}
		...
		case BACKSPACE: /*Hapus char*/
			...
			prevEOL = findEndOfLine(text, y - 1);
			if (x > 0)
			{ /*Jika cursor tidak di awal baris*/
				x--;
				for (int i = x; i < curEOL; i++)
				{ /*Geser char ke kiri*/
					text[y][i] = text[y][i + 1];
				}
				moveTo(y, x);
			...
			{									/*Jika cursor berada di awal baris*/
				...
				{ /*Jika terdapat ruang*/
					if (curEOL > remainder)
					{ /*Jika ruang di baris atas tidak cukup untuk menampung char di baris yang ditunjuk cursor*/
					...
					{ /*Jika ruang di baris atas cukup menampung char di baris yang ditunjuk cursor*/
				...
				{ /*Jika tidak terdapat ruang di baris atas*/
					if (curEOL != 0)
					{ /*Jika baris saat ini tidak kosong*/
						...
						curEOL = findEndOfLine(text, y);
						if (curEOL == 0)
						{ // Jika char di baris ini habis maka hapus baris
					...
					{ /*Jika baris yang ditunjuk cursor kosong */
						if (y < amountOfLine)
						{ /*Jika terdapat baris lain di bawah cursor*/
							delLine(y, &amountOfLine, text);
							clearScreenToBottom();
							print2DArr(text, y, amountOfLine);
							x = prevEOL;
						...
						{ /*Jika tidak terdapat baris lain di bawah cursor*/
							y--;
							x = prevEOL;
							amountOfLine--;
							text[y + 1][0] = NULL;
		...
		default:
			...
			else if (x >= MAXKOLOM && amountOfLine < MAXBARIS)
			{ // Apabila melebihi batas kolom
				amountOfLine++;
				y++;
				x = 0;
				moveTo(y, x);
